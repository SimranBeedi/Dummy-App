...
def login_shopkeeper():
    if request.method == 'POST':
        ...
        if shopkeeper:
            stored_password_hash = shopkeeper[2]
            is_approved = shopkeeper[8]
            ...
            if bcrypt.check_password_hash(stored_password_hash, password):
                if is_approved is None:
                    ...
                    session['username'] = shopkeeper[1]  # Set username for shopkeeper
                    session['shopkeeper_id'] = shopkeeper[0]
...
@shopkeeper_bp.route('/logout', methods=['GET'])
def logout_shopkeeper():
    ...
    return redirect(url_for('home'))  # Redirect to the home page or a login page
