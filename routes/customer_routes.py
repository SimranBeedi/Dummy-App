    ...
    msg = Message(
        ...
        <body>
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f9f9f9;">
                ...
                <p>If you need any assistance, feel free to contact our support team at <a href="mailto:buydirect24info@gmail.com" style="color: #2b85d8;">buydirect24info@gmail.com</a>.</p>
        ...
        if user:
            ...
            if bcrypt.check_password_hash(stored_password_hash, password):
                if user['is_verified'] == 1:
                    session['user_id'] = user['id']  # Updated session key
                    session['customer_name'] = user['username']  # Changed from username to customer_name
                    session['user_type'] = 'customer'  # Set user type to 'customer'
...
def view_cart():
    ...
    try:
        ...
        shipping = Decimal(0) if subtotal > 600 else Decimal(40) if has_items else Decimal(0)
    ...
    return render_template('customer/view_cart.html', cart_items=cart_items, subtotal=subtotal, shipping=shipping, gst=total_gst, total=total, cart_count=cart_count, has_items=has_items)
...
def process_payment():
    ...
    try:
        ...
        if order_id:
            ...
            cursor = mysql.connection.cursor()
            try:
                cursor.execute("DELETE FROM cart_items WHERE customer_id = %s", (session['customer_id'],))
                mysql.connection.commit()
            except MySQLdb.Error as e:
                mysql.connection.rollback()
                app.logger.error(f"Failed to clear cart: {str(e)}")
                flash('An error occurred while clearing the cart.', 'danger')
            finally:
                cursor.close()
